...
/// Main screen widget that handles tab navigation
...
/// Base class for insight pages to enforce common structure
  ...
  Widget _buildNewsItem(String item) {
    return Padding(
      ...
      child: Row(
        children: [
          Icon(Icons.update,
              size: 12, color: AppTheme.greenAccentColor),
...
class P2PInsightsPage extends BaseInsightsPage {
  ...
  List<InsightMetric> getMetrics() => const [
    InsightMetric('Average Returns', '12%'),
    InsightMetric('Highest Return', '20%'),
    InsightMetric('Default Rate', '2%'),
  ];

  @override
  List<TrendItem> getTrends() => const [
    TrendItem('Bridging the credit gap in underserved markets'),
    TrendItem('AI-driven risk assessment for smarter lending decisions'),
    TrendItem('Empowering investors with diversified lending portfolios'),
    TrendItem('Institutional adoption increasing P2P liquidity and trust'),
  ];

  @override
  List<String> getNews() => const [
    'RBI unveils investor-friendly reforms for P2P lending',
    'P2P lending market poised to cross \$10 billion by 2025',
    'AI-driven credit analysis boosts borrower approvals by 30%',
    'Record low default rates strengthen investor confidence',
    'Millennials drive 60% of new P2P lending activity',
  ];
}
...
class InvoiceInsightsPage extends BaseInsightsPage {
  ...
  List<TrendItem> getTrends() => const [
    ...
    TrendItem('Lower default rates boosting investor trust'),
  ...
  List<String> getNews() => const [
    ...
    'Record investor participation in MSME invoices',
...
class NCDInsightsPage extends BaseInsightsPage {
  ...
  @override
  List<TrendItem> getTrends() => const [
    TrendItem('Longer tenures attract stable investors'),
    TrendItem('High yields drive demand for unsecured NCDs'),
    TrendItem('Green NCDs gain popularity among ESG investors'),
  ...
  List<String> getNews() => const [
    ...
    'Digital platforms make NCDs accessible to retail investors',
